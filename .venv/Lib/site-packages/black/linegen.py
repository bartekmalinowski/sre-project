...
class LineGenerator(Visitor[Line]):
    ...
    def visit_fstring(self, node: Node) -> Iterator[Line]:
        ...
        if "\\" in string_leaf.value and any(
            "\\" in str(child) for child in node.children if child.type == syms.fstring_replacement_field
        ):
    ...
    if node.children[0].type == token.AWAIT and len(node.children) > 1:
        if node.children[1].type == syms.atom and node.children[1].children[0].type == token.LPAR:
            ...
            if isinstance(bracket_contents, Node) and (
                ...
                or any(
                    isinstance(child, Leaf) and child.type == token.DOUBLESTAR for child in bracket_contents.children
                )
            ):
