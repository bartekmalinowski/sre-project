        ...
        def make_naked(string: str, string_prefix: str) -> str:
            ...
            if "f" in string_prefix:
                ...
                debug_expressions_contain_visible_quotes = any(
                    re.search(r".*[\'\"].*(?<![!:=])={1}(?!=)(?![^\s:])", expression) for expression in f_expressions
                )
    ...
    def _validate_msg(line: Line, string_idx: int) -> TResult[None]:
        ...
        for leaf in line.leaves[string_idx:]:
            ...
            if (
                ...
                and ("'" in leaf.value[len(prefix) + 1 : -1] or '"' in leaf.value[len(prefix) + 1 : -1])
            ):
                return TErr("StringMerger does NOT merge f-strings with different quote types" " and internal quotes.")
...
class BaseStringSplitter(StringTransformer):
    ...
    def do_match(self, line: Line) -> TMatchResult:
        ...
        assert len(string_indices) == 1, (
            f"{self.__class__.__name__} should only find one match at a time, found" f" {len(string_indices)}"
        )
