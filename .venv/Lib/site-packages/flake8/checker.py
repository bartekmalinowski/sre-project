...
def _mp_prefork(plugins: Checkers, options: argparse.Namespace) -> Generator[None]:
...
class Manager:
    ...
    def _job_count(self) -> int:
        ...
        if utils.is_using_stdin(self.options.filenames):
            LOG.warning(
                "The --jobs option is not compatible with supplying " "input using - . Ignoring --jobs arguments."
            )
            return 0
...
class FileChecker:
    ...
    def _extract_syntax_information(exception: Exception) -> tuple[int, int]:
        ...
        if column > 0 and token and isinstance(exception, SyntaxError) and len(token) == 4:  # Python 3.9 or earlier
