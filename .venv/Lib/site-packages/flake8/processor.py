...
class FileProcessor:
        ...
        for param, required in parameters.items():
            ...
            try:
                ...
                if required:
                    ...
                    LOG.warning(
                        'Plugin requested optional parameter "%s" ' "but this is not an available parameter.",
                        param,
                    )
        return ret
    ...
    def should_ignore_file(self) -> bool:
        ...
        if not self.options.disable_noqa and any(defaults.NOQA_FILE.match(line) for line in self.lines):
            ...
            LOG.warning(
                "Detected `flake8: noqa` on line with code. To ignore an " "error on a line use `noqa` instead."
            )
            return False
        else:
            return False
