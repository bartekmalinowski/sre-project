...
class Definition(Binding):
    ...
    def redefines(self, other):
        return super().redefines(other) or (isinstance(other, Assignment) and self.name == other.name)
...
class Checker:
    ...
    def addBinding(self, node, value):
        ...
        if value.name not in self.scope or not isinstance(value, Annotation):
            if isinstance(value, NamedExprAssignment):
                ...
                scope = next(scope for scope in reversed(self.scopeStack) if not isinstance(scope, GeneratorScope))
    ...
    def DICT(self, node):
        ...
        for key in duplicate_keys:
            ...
            values = collections.Counter(convert_to_value(node.values[index]) for index in key_indices)
